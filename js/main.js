var container, scene, camera, controls;
			...
			//SNOW GEOMETRY
			var n = 100000; // Numero particelle
			...
			for(i = 0; i < n; i++){
			    ...
			    // Per la casa
			    if( init_pos_x[i] > 0 && init_pos_z[i] > 5 &&
			    	init_pos_x[i] < 15 && init_pos_z[i] < 20){

			    	min_level[i] = -32.5 + 50 - Math.abs(init_pos_x[i]-7.5)*0.9;	
			    }

			    // Per l'albero
			     if( (init_pos_x[i] + 5)**2 + (init_pos_z[i] + -22)**2 < 16){

			    	min_level[i] = -32.5 + 40
			    	+(4 - Math.sqrt( (init_pos_x[i]-5)**2 + (init_pos_z[i]- -22)**2 ))*0.3;	
			    }
			} 
			//count = 0;
